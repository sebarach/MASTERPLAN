												...
												<?php
													foreach ($campanas as $c) {
														if($c["activo"]==1){
															...
															if($perm>=4){
																if($perm!=2 || $perm!=1){
																	echo "<div class='btn-group-vertical'><button type='button' class='btn btn-sm btn-outline-theme' onclick='campana(\"".base64_encode($c["id_campana"])."\")' data-toggle='modal' data-target='#modal10' style='margin-top:0;' title='Editar'><i class='fa  fa-pencil'></i></button>";
																}
															}
															for($i=0; $i<count($modulos); $i++) {
																if($modulos[$i]=="Acciones"){
																	echo "<button type='button' class='btn btn-outline-theme btn-sm' onclick='acciones(\"".base64_encode($c["id_campana"])."\")' title='Acciones'><i class='mdi mdi-chemical-weapon'></i></button>
																	</div>";
																}
																if($modulos[$i]=="Informes"){
																	echo "<div class='btn-group-vertical'><button type='button' class='btn btn-outline-theme btn-sm' onclick='informes(\"".base64_encode($c["id_campana"])."\")' title='Reportes'><i class='fa  fa-dashboard '></i></a>";
																}												
															}
															...
															echo "<div class='btn-group' ><button type='button' class='btn btn-outline-theme btn-sm' onclick='biblio(\"".base64_encode($c["id_campana"])."\")' title='Biblioteca'><i class='fa fa-book'></i></button></div>";
															echo "</div></td>";
...
<script type="text/javascript">
	...
	$.ajax({
		url:"<?php echo base_url("empresas/informes");?>",
		type: "POST",
		data: "id="+id,
		success: function(data){
			window.location.href='<?php echo base_url("informes/reporte");?>';
		}
	});
}
