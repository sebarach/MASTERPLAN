			...
			if(isset($_SESSION["empresa"])){
				if(isset($_SESSION["campana"])){
					if(isset($_SESSION["accion"])){
						...
						if(base64_decode($_SESSION["empresa"])==9){
							$c=array("ID_Campana","ID_Accion","ID_Cliente","Fecha_Programada","Medicion","Acuerdo","Implementado","ID_Motivo","Clasificacion","Tipo_Acuerdo","Fecha_Inicio_Acuerdo","Fecha_Fin_Acuerdo");
						} else {
		...
		if(isset($_FILES['ex_master'])){
			...
			if($ex=="xls" || $ex=="xlsx"){
				...
				if($file->getSheetCount()>2){ 
					...
					$mensaje=$this->hoja2($file);
				} else {
					$this->load->model("clienteinfo");
					if($this->hoja1($file)!="" && $this->hoja2($file)!=""){
						$mensaje=$this->hoja1($file)."<br>".$this->hoja2($file);
					} else if($this->hoja1($file)!=""){
						$mensaje=$this->hoja1($file);
					} else if($this->hoja2($file)!=""){
						$mensaje=$this->hoja2($file);
					} else {
						$this->addhoja1($file);
						$this->addhoja2($file);
