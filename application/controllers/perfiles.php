...
class perfiles extends CI_Controller {
	...
	public function index(){
		if(isset($_SESSION["sesion"])){
			...
			$data["levels"]=array('Empresas','Clientes','Campanas','Acciones','Informes','Motivos_Grupos','Motivos','Regiones','Usuarios','UserLevels','Clientes_Informacion','Tipos_Tareas','Solicitud_ot');
	...
	public function agregar(){
		if(isset($_SESSION["sesion"])){
			...
			if((!isset($_POST["txt_modulosadd"])) || count($_POST["txt_modulosadd"])<0){
				$vacios+=1;
			}
			if($vacios!=0){
				...
				$permisos=array();
				for ($i=0; $i < count($_POST["txt_modulosadd"]); $i++) { 
					$suma=0;
					...
					$permisos[$i]=$_POST["txt_modulosadd"][$i]."/".$suma;
				}
	...
	public function editar(){
		if(isset($_SESSION["sesion"])){
			...
			if((!isset($_POST["txt_modulos"])) || count($_POST["txt_modulos"])<0){
				$vacios+=1;
			}
			if($vacios!=0){
				$data["mensaje"]="Perfil no pudo ser agregado, intente nuevamente";
				...
				for ($i=0; $i < count($_POST["txt_modulos"]); $i++) { 
					$suma=0;
					if(isset($_POST["chck_view1".$i])){
						$suma+=$_POST["chck_view1".$i];
					} else {
						$suma+=0;
					}
					if(isset($_POST["chck_add1".$i])){
						$suma+=$_POST["chck_add1".$i];
					} else {
						$suma+=0;
					}
					if(isset($_POST["chck_edit1".$i])){
						$suma+=$_POST["chck_edit1".$i];
					} else {
						$suma+=0;
					}
					if(isset($_POST["chck_delete1".$i])){
						$suma+=$_POST["chck_delete1".$i];
					} else {
						$suma+=0;
					}
					$permisos[$i]=$_POST["txt_modulos"][$i]."/".$suma;
				}
