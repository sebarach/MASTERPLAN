...
class clientesinfo extends CI_Controller {
			...
			if(isset($_SESSION["empresa"])){
				if(isset($_SESSION["campana"])){
					if(isset($_SESSION["accion"])){
						...
						if(base64_decode($_SESSION["empresa"])==9){
							$c=array("ID_Campana","ID_Accion","ID_Cliente","Fecha_Programada","Medicion","Acuerdo","Implementado","ID_Motivo","Tipo_Acuerdo","Fecha_Inicio_Acuerdo","Fecha_Fin_Acuerdo");
						} else {
		...
		if(isset($_SESSION["sesion"])){
			if(isset($_SESSION["empresa"])){
				if(isset($_SESSION["campana"])){
					if(isset($_SESSION["accion"])){
						$this->load->library('phpexcel');
						$tipo = PHPExcel_IOFactory::identify("archivos/Macro_Master.xls");
						$reader = PHPExcel_IOFactory::createReader($tipo);
						$file= $reader->load("archivos/Macro_Master.xls");
						$file->setActiveSheetIndex(0);
						$file->getActiveSheet()->setCellValueByColumnAndRow(0,2,base64_decode($_SESSION["campana"]));
						$file->getActiveSheet()->setCellValueByColumnAndRow(1,2,base64_decode($_SESSION["accion"]));
						ob_end_clean();
						$excel_writer = PHPExcel_IOFactory::createWriter($file, 'Excel5');
						// header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
						// header('Content-Disposition: attachment;filename="Macro_Master.xls"');
						// header('Cache-Control: max-age=0');		
						$excel_writer->save('archivos/Macro_Master.xls');
						redirect(base_url("archivos/Macro_Master.xls"));
					} else {
						redirect(base_url("accciones"));
					}
				} else {
				...
				if(isset($_SESSION["campana"])){
					if(isset($_SESSION["accion"])){
						...
						if($vacios!=0){
							...
							$data["fecha_programada"]=date("Ymd",strtotime($_POST["txt_fecha_programada"]));
				...
				if(isset($_SESSION["campana"])){
					if(isset($_SESSION["accion"])){
						...
						if($vacios!=0){
							...
							$data["implementado"]="0";
							$data["id_motivo"]="99";
							$data["fecha_programada"]=date("Ymd",strtotime($_POST["txt_fecha_programada_i"]));
							if(base64_decode($_SESSION["empresa"])==9){
			...
			if(isset($_SESSION["empresa"])){
				if(isset($_SESSION["campana"])){
					if(isset($_SESSION["accion"])){
						...
						if($vacios!=0){
							$_SESSION["mensaje"]="Debe seleccionar una de las opciones. (INGRESAR MASTER o EDITAR MASTER)";
						} else {
	...
	function addmaster(){
		...
		if(isset($_FILES['ex_master'])){
			...
			if($ex=="xls" || $ex=="xlsx"){
				...
				if($file->getSheetCount()>2){ 
				...
				} else if($file->getSheetCount()==1){
					if($this->hoja2($file,0)!=""){
						$mensaje=$this->hoja2($file,0);
					} else {
						$this->load->model("clienteinfo");
						$this->addhoja2($file,0);
					...
					$this->load->model("clienteinfo");
					if($this->hoja1($file)!="" && $this->hoja2($file,1)!=""){
						$mensaje=$this->hoja1($file)."<br>".$this->hoja2($file,1);
					} else if($this->hoja1($file)!=""){
						$mensaje=$this->hoja1($file);
					} else if($this->hoja2($file,1)!=""){
						$mensaje=$this->hoja2($file,1);
					} else {
						$this->addhoja1($file);
						$this->addhoja2($file,1);
						$add=$this->clienteinfo->validarmasterplanadd();
						if($add["respuesta"]==1){
							$mensaje="Carga de Master exitosa.";
							$carpeta="archivos/fotos/".base64_decode($_SESSION["accion"]);
	...
	function hoja1($file){
		...
		if(count($erroresh1)>0){
			...
			$h1="Errores Hoja1: ".substr($h1,0,-2)."<br>";
		}
		return $h1;
	}


	function hoja2($file,$id){
		//Validar Hoja2
		$file->setActiveSheetIndex($id);
		$erroresh2=array();
		$e=0; $column=0;
		$letras2=PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($file->setActiveSheetIndex($id)->getHighestColumn()));
		for ($i=1; $i <= $file->setActiveSheetIndex($id)->getHighestRow(); $i++) {
			for ($col='A'; $col != $letras2; $col++) {
				if($i!=1){
					if($file->getActiveSheet()->getCell(''.$col.$i)->getCalculatedValue()===NULL){
						if($this->checkrow($file,$id,$i)){
							$column=1;
							break;
						} else {
							$erroresh2[$e]=$col.$i;
		...
		if(count($erroresh2)>0){
			...
			$h2="Errores Hoja2: ".substr($h2,0,-2)."<br>";
		} 
		return $h2;
	}


	function addhoja1($file){
		$m=0;
		for ($i=1; $i <= $file->setActiveSheetIndex(0)->getHighestRow(); $i++) {
			if($i!=1){
				$data=array();
				for ($col='A'; $col != PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($file->setActiveSheetIndex(0)->getHighestColumn())); $col++) {
					if($file->getActiveSheet()->getCell(''.$col.$i)->getCalculatedValue()!==NULL){
						...
						if(PHPExcel_Shared_Date::isDateTime($file->getActiveSheet()->getCell(''.$col.$i))) {
							...
							$data[$id]=str_replace("'",'"' , $file->getActiveSheet()->getCell(''.$col.$i)->getCalculatedValue());
						}										 				
					} else {
						$m=1;
						break;
	...
	function addhoja2($file,$id){
		$m=0;
		for ($i=1; $i <= $file->setActiveSheetIndex($id)->getHighestRow(); $i++) {
			if($i!=1){
				$data=array();
				for ($col='A'; $col != PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($file->setActiveSheetIndex($id)->getHighestColumn())); $col++) {
					if($file->getActiveSheet()->getCell(''.$col.$i)->getCalculatedValue()!==NULL){	
						$cab=strtolower($file->getActiveSheet()->getCell(''.$col.'1')->getCalculatedValue());
						if(PHPExcel_Shared_Date::isDateTime($file->getActiveSheet()->getCell(''.$col.$i))) {
							$timestamp = PHPExcel_Shared_Date::ExcelToPHP($file->getActiveSheet()->getCell(''.$col.$i)->getValue());
							$data[$cab]= date("Ymd",$timestamp);
						} else {
							$data[$cab]=str_replace("'",'"' , $file->getActiveSheet()->getCell(''.$col.$i)->getCalculatedValue());
						}
					} else {
						$m=1;
						break;
